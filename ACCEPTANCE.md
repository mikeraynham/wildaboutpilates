...
`public/` (`hugomods/hugo:exts`, Hugo v0.154.5+extended) and diffed
...
2. **CSS BOM + sourceMappingURL.** Confirmed. `scratch/jekyll-baseline/css/screen.css`
   ...
   `/*# sourceMappingURL=screen.css.map */`); `public/css/screen.css` is
...
(`grep -rl generator public/` returns nothing), and
...
- **G — `/blog/` list excerpts leaking full post content.** `<!--more-->`
  ...
  `public/blog/index.html` is now 169 lines (was leaking substantially
...
produced by this build; `public/` is gitignored). A clean rebuild
...
`static/images/` and served as-is, so the sized files stayed
...
value pointing at an original in `assets/images/`, and Hugo regenerates
...
`static/images/` (119 files in all: each image's sized variants plus the
one source copy that was relocated into `assets/images/`). The generated
...
`data/classes.yml`) was saved once through the pinned Sveltia CMS
