...
Every page (`content/*.md`, `content/blog/*.md`) is built from a
`blocks` list.  There is no free-text page body: each block is a typed
entry, rendered by the matching partial in `layouts/partials/blocks/`,
and the CMS (Sveltia, `static/admin/config.yml`) offers only the block
...
`layouts/partials/blocks/`; an unknown type fails the build.
...
`data/classes.yml`), and both the Class times block and the Venue
...
The original lives in `assets/images/`. When you upload or Replace a
...
- **Site-chrome images are separate.** A few images are not block images: the
  ...
  navigation-icon sprite. These live in `static/images/` and are served
